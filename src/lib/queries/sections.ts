...
export async function getSections(supabase: DbClient, includeContents = false) {
  const selectClause = includeContents ? '*, contents(*)' : '*';
  return supabase.from('sections').select(selectClause).order('priority');
}

export async function getSectionById(supabase: DbClient, id: number) {
  return supabase.from('sections').select('*').eq('id', id).maybeSingle();
}
...
export async function deleteSection(supabase: DbClient, id: number) {
  return supabase.from('sections').delete().eq('id', id).select('id').maybeSingle();
}

export async function reorderSections(supabase: DbClient, items: ReorderInput) {
  const now = new Date().toISOString();
  const results = await Promise.all(
    items.map(({ id, priority }) => supabase.from('sections').update({ priority, updated_at: now }).eq('id', id))
  );
