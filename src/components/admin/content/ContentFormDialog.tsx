...
const ContentFormDialog: FC<ContentFormDialogProps> = ({
  ...
  const [snap, setSnap] = useState<{ open: boolean; initial: Content | null | undefined }>({ open, initial });

  if (open !== snap.open || initial !== snap.initial) {
    setSnap({ open, initial });
    if (open) {
  ...
  const canSubmit = !!title.trim() && !!subtitle.trim() && !!dateRange.trim() && !!description.trim() && !submitting;
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>
            {mode === 'add' ? '컨텐츠 추가' : '컨텐츠 편집'}
            {sectionName ? ` · ${sectionName}` : ''}
          </DialogTitle>
          <DialogDescription>
            {mode === 'add' ? '섹션에 표시할 새 항목을 입력하세요.' : '컨텐츠 내용을 수정하세요.'}
          </DialogDescription>
        </DialogHeader>
