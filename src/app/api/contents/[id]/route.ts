...
export const GET = withAdmin<Params>(async (_req, { params }) => {
  ...
  const { data, error } = await getContentById(supabase, numId);
  if (error) {
    console.error(error);
...
export const PUT = withAdmin<Params>(async (req, { params }) => {
  ...
  const supabase = await createServerClient();
  const { data, error } = await updateContent(supabase, numId, parsed.data);
  if (error) {
    console.error(error);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
...
export const DELETE = withAdmin<Params>(async (_req, { params }) => {
  ...
  const supabase = await createServerClient();
  const { data, error } = await deleteContent(supabase, numId);
  if (error) {
    console.error(error);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
